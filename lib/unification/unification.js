...
function andUnify(unifiedTerms){
  ...
  var unifiedTermsStrings = unifiedTerms.map((ut) => {
    ...
    if(intersect.size() > 0){
      ...
      for(var j = 0; j < interSectList.length; j++){
        ...
        if(reunion === null){
         ...
         var reUnioned = JSON.stringify(reunion);
          if(unifiedTermsStrings.indexOf(reUnioned) < 0){
            unifiedTerms.push(reunion);
            unifiedTermsStrings.push(reUnioned)
          }
        }
      }
    }
